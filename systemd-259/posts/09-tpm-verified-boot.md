...
With v259 the new NvPCRs are used to measure two new things: we measure the SMBIOS/Devicetree product UUID, and we measure the cryptsetup unlock mechanism used. We soon want to extend that, and for example measure the root hash + sig of every single DDI activated on the system. With that we have a pretty complete trail of everything going on on the system.
...
Well, the pc profile says there should be 76 nvindexes allocatable. But it's not entirely clear how many of those shall be accessible in nvextend mode, and if you store very large data in other nvindexes then there might not be enough data for the nvpcrs anymore. That said, nvpcrs require no data storage, just metadata storage (since they reset to 0 on boot), and thus they should be very space efficient...
