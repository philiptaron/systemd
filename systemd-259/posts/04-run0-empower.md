...
For this one I am simply going to top-post [@daandemeyer](https://mastodon.social/@daandemeyer)'s story about `run0`'s new `--empower` switch, which gives your process capability + polkit privileges, without changing your user ID.
Very powerful stuff.
...
This leaves auth to pk, unifying auth at one place hence: pk.
Regular users by default get to use it only after the usual pk auth query.
