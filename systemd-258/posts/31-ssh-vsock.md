...
That's some 32bit value that basically plays the role of an IP addresses, except for AF_VSOCK rather than AF_INET.
...
The `.socket` units that bind the AF_VSOCK will now call a tiny new tool `systemd-ssh-issue` that will generate…
...
The files in `/run/issue.d/*.issue` are shown by the `login` process that is responsibly on Linux to show the login prompt.
Or in other words: when you get the console login prompt on a VM system you'll now also get a suggestion how to log into it via ssh-over-AF_VSOCK as alternative means.
