---
layout: post
title: "homectl adopt and register"
date: 2025-08-27
---
...
First of all there's `homectl adopt`. You just pass a path to an existing `*.home` LUKS disk image, or a `*.homedir` home directory, and it will make it available locally for login (assuming it carries the right signatures accepted by the local system). The same could already be achieved by simply linking the `.home`/`.homedir` path into `/home/`, which also causes `systemd-homed` to pick it up, but `homectl adopt` works without that, the actual home dir remains wherever it is, and `/home/` is not modified.
...
The two commands complement each other: one takes the home dir itself, the other JSON record instead, both add an entry to the list of users accessible locally.
...
The `register` command is useful when doing something like the SMB sharing of a home dir, as described in episode 38. For example: `homectl inspect -E <user> | ssh <otherhost> homectl register` is a powerful command for propagating a user account from one host to another.
