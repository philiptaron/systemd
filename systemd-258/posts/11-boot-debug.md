...
11️⃣ Here's the 11th post highlighting key new features of the upcoming v258 release of systemd. [#systemd258](https://mastodon.social/tags/systemd258)
...
No but they are documented in the `debug-generator` man page.
...
You certainly can. But only really in very simple setups. Thing is anything non-trivial really should do disk encryption for the rootfs and that implies some userspace. Either to do the tpm stuff, or to query user for pw, or to do fido2. Its not going to happen that the kernel will do all that itself.
