...
This is used for a number of uses, including `run0` (so that the target shell runs within a proper PAM session with all PAM modules in effect), and `user@*.service` (the system service that contains the per-user service manager).
...
> **JigenDaisukeJr** I said it before and I'll say it again - homed is one of the best ways to manage user's home in modern systems. And what is the "lingering mode", btw?
