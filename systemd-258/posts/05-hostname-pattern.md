---
layout: post
title: "Hostname pattern matching with `hostname(5)`"
date: 2025-05-28
---
...
Often deployments use technologies like `DHCP` leases or cloud provisioning tools to set hostnames like this.
...
Note that you can use as many question marks as you like (well, you probably shouldn't use more than 63 characters for the whole hostname, since that's the maximum length of a DNS label and also the limit the Linux kernel enforces on the local hostname).
Of course, keep the birthday paradox in mind when picking the number of question marks, since you probably want to avoid accidental hostname clashes in your fleet.
There's a reason why UUIDs are 128-bit long (i.e., 32 hex characters)...
