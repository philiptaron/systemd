...
Why is this useful? This is used by `systemd-networkd-wait-online`'s new `--dns` flag. If specified it will not only wait until a suitable IP configuration shows up, but also wait until a DNS configuration is acquired as well, making the whole logic more robust regarding dynamic network configuration.

And in case you wonder what `systemd-networkd-wait-online` is good for: on systems that boot from the network it makes sense to delay attempts to contact the boot servers until the moment networking is fully established (but no longer), so that no time is wasted on pointless retry cycles unnecessarily.
