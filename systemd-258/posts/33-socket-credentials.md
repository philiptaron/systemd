...
The Linux kernel recently gained two new socket options (as in `getsockopt()` commands) for AF_UNIX sockets: `SO_PASSPIDFD` and `SO_PASSRIGHTS`. The former complements `SO_PASSCREDS` (which a receiver can set and ensures that sender UID/GID/PID information identifying the sender is sent along with payload on the socket). It simply sends a stable, new-style `pidfd` reference instead of a plain pid, along with the payload.
