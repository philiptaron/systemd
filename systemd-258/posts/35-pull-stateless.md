...
3️⃣5️⃣ Here's the 35th post highlighting key new features of the upcoming v258 release of systemd. [#systemd258](https://mastodon.social/tags/systemd258) [#systemd](https://mastodon.social/tags/systemd)
...
This example disables authentication (the `verify=no` is doing that), which sounds dangerous, but doesn't really have to be, if the referenced disk image is a proper DDI, i.e. carries Verity information and a signature for it.
...
Yeah, `mkosi` only outputs DDIs. If you want something more raw (i.e. just turn some pre-prepared directory tree into a DDI) use `systemd-repart -P`.
