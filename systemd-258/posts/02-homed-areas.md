...
In order to create such an area anew you can simply call `mkdir -p ~/Areas` and `cp -av /etc/skel ~/Areas/mynewarea`.
...
My personal use case for this goes something like this: I regularly build disk images for VMs of different distributions on my host, for building and testing systemd and other stuff.
...
But there's no reason for not supporting that properly besides "Lennart didn't find the time to make it work yet".
