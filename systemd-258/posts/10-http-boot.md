...
Moreover the networking implementation in UEFI mode might not be the most efficient.
...
We have extended the Boot Loader Spec Type #1 files (those `.conf` files in `/boot/loader/entries`) to support a new stanza:

`"uki-url"`.
This is used in a very similar way to the current `"linux"` stanza: you point it to a kernel to boot.
In this case however it can be an URL where a UKI is placed.
