...
The way the records are usually understood in UNIX is that the username ↔ record ↔ UID relationships are bijective, i.e. there's exactly one user name for a record, and one UID for it and vice versa, but as far as I can see this is never clarified as part of POSIX.
...
Use case: my user `lennart` now has an alias `l`, so that I save 6 keypresses per login! Yay!
...
With this, back/forth translations *are* reversible: if you lookup a user record by an alias name, then lookup the user record by its UID, it *will* list the alias name.

---

> **What kind of races are you referring to?**
>
> The lookups are always done in parallel to all backends, and the first one replying wins, under the assumption that user name/UID conflicts have been dealt with already before lookup time, i.e. at definition time.

> **On conflict resolution:**
>
> `userdb` *expressly* makes no attempt to resolve conflicts. That can only fail and hide problems. If there are conflicts then they need to be dealt with *before* making the records available, not after.
>
